 ...
 * Description: Adds referral reward history, payout marking, daily referral tracking, referred-user details, payout summaries, CSV export, and frontend popup referral cards, fraud flags, duplicate M-Pesa blocking, and self-referral protection for the existing BuyTap referral system.
 * Version: 5.2.0
...
class BuyTap_Referral_Reward_Reports_V5 {
    ...
    public function render_admin_page() {
        ...
        <div class="wrap buytap-referral-rewards">
            ...
            <table class="widefat striped">
                <thead><tr><th>Referrer</th><th>Email</th><th>M-Pesa</th><th>Referral Code</th><th>Total</th><th>Pending</th><th>Flagged</th><th>Redeemed</th><th>Pending Amount</th><th>Action</th></tr></thead>
                <tbody>
            ...
            <table class="widefat striped">
                <thead><tr><th>Date</th><th>Time</th><th>Referrer</th><th>Referrer M-Pesa</th><th>Referred Person</th><th>Referred Email</th><th>Reward</th><th>Status</th><th>Fraud Flags</th><th>Redeemed At</th><th>Code Used</th></tr></thead>
                <tbody>
                <?php if (empty($rows)): ?>
                ...
                <?php else: foreach ($rows as $row): ?>
                    <tr>
                        ...
                        <td><?php echo $row['reward_status'] === 'redeemed' ? '<strong style="color:#15803d;">Redeemed</strong>' : ($row['reward_status'] === 'flagged' ? '<strong style="color:#dc2626;">Flagged</strong>' : '<strong style="color:#b45309;">Pending</strong>'); ?></td>
                        <td><?php echo empty($row['fraud_flags']) ? '<span style="color:#15803d;">Clear</span>' : '<strong style="color:#dc2626;">' . esc_html(implode(', ', $row['fraud_flags'])) . '</strong>'; ?></td>
    ...
    public function export_csv() {
        ...
        fputcsv($out, ['Date', 'Time', 'Referrer Name', 'Referrer Email', 'Referrer M-Pesa', 'Referral Code', 'Referred Name', 'Referred Email', 'Referred M-Pesa', 'Code Used', 'Reward Currency', 'Reward Amount', 'Reward Status', 'Fraud Flags', 'Redeemed At', 'Batch']);

        foreach ($rows as $row) {
            fputcsv($out, [$row['date'], $row['time'], $row['referrer_name'], $row['referrer_email'], $row['referrer_mobile'], $row['referrer_code'], $row['referred_name'], $row['referred_email'], $row['referred_mobile'], $row['promo_code_used'], $currency, number_format(floatval($row['reward_amount']), 2, '.', ''), $row['reward_status'], implode(' | ', $row['fraud_flags']), $row['rewarded_at'], $row['rewarded_batch']]);
        }
    ...
    private function render_frontend_rows($rows, $currency, $show_email) {
        ...
        if (empty($rows)) {
            ...
            foreach ($rows as $row) {
                $status = in_array($row['reward_status'], ['redeemed', 'flagged'], true) ? $row['reward_status'] : 'pending';
                $label = $status === 'redeemed' ? 'Redeemed' : ($status === 'flagged' ? 'Under Review' : 'Pending');
                ?>
