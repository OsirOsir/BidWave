 ...
 * Description: Adds referral reward history, payout marking, daily referral tracking, referred-user details, payout summaries, CSV export, and frontend popup referral cards for the existing BuyTap referral system.
 * Version: 5.1.0
...
class BuyTap_Referral_Reward_Reports_V5 {
    ...
    private function referral_status_for_user($user_id) {
        ...
        return $redeemed === 'yes' ? 'redeemed' : 'pending';
    }
    ...
    public function render_admin_page() {
        ...
        <div class="wrap buytap-referral-rewards">
            ...
            <p>This report uses existing referral fields: <code>my_referral_code</code> and <code>promo_code</code>. Rewards are calculated using the rate active when each referred user registered.</p>
            ...
            <table class="widefat striped">
                <thead><tr><th>Referrer</th><th>Email</th><th>M-Pesa</th><th>Referral Code</th><th>Total</th><th>Pending</th><th>Redeemed</th><th>Pending Amount</th><th>Action</th></tr></thead>
                <tbody>
            ...
            <table class="widefat striped">
                <thead><tr><th>Date</th><th>Time</th><th>Referrer</th><th>Referrer M-Pesa</th><th>Referred Person</th><th>Referred Email</th><th>Reward</th><th>Status</th><th>Redeemed At</th><th>Code Used</th></tr></thead>
                <tbody>
                <?php if (empty($rows)): ?>
                ...
                <?php else: foreach ($rows as $row): ?>
                    <tr>
                        ...
                        <td><?php echo $row['reward_status'] === 'redeemed' ? '<strong style="color:#15803d;">Redeemed</strong>' : '<strong style="color:#b45309;">Pending</strong>'; ?></td>
    ...
    public function export_csv() {
        ...
        fputcsv($out, ['Date', 'Time', 'Referrer Name', 'Referrer Email', 'Referrer M-Pesa', 'Referral Code', 'Referred Name', 'Referred Email', 'Referred M-Pesa', 'Code Used', 'Reward Currency', 'Reward Amount', 'Reward Status', 'Redeemed At', 'Batch']);

        foreach ($rows as $row) {
            fputcsv($out, [$row['date'], $row['time'], $row['referrer_name'], $row['referrer_email'], $row['referrer_mobile'], $row['referrer_code'], $row['referred_name'], $row['referred_email'], $row['referred_mobile'], $row['promo_code_used'], $currency, number_format(floatval($row['reward_amount']), 2, '.', ''), $row['reward_status'], $row['rewarded_at'], $row['rewarded_batch']]);
        }
    ...
    private function render_frontend_rows($rows, $currency, $show_email) {
        ...
        if (empty($rows)) {
            ...
            foreach ($rows as $row) {
                $status = $row['reward_status'] === 'redeemed' ? 'redeemed' : 'pending';
                $label = $status === 'redeemed' ? 'Redeemed' : 'Pending';
                ?>
