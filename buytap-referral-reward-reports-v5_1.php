...
class BuyTap_Referral_Reward_Reports_V5 {
    ...
    public static function activate() {
        ...
        // ── NEW: default to enabled on first activation ───────
    ...
    // ══════════════════════════════════════════════════════════
    //  NEW: Rewards Status helpers
    //  These ONLY control the fixed plugin rewards.
    //  The main BuyTap order referral bonus system is untouched.
    // ══════════════════════════════════════════════════════════

    /**
     * Returns true when the fixed plugin reward payouts are active.
     * Has no effect on the main BuyTap order referral bonus system.
     */
    ...
    /**
     * Save the Enabled / Disabled toggle from the admin form.
     * Only admins can call this; protected by nonce + capability check.
     */
    ...
    private function get_current_reward_amount() {
        // ── When rewards are disabled, display amount as 0 ────
    ...
    private function get_reward_for_datetime($datetime) {
        // ── When rewards are disabled, all pending reward values
        //    are treated as 0. Already-redeemed rows use their
        //    stored meta amount (set at payout time) so history
        //    remains untouched. See get_referred_users() below.
        // ─────────────────────────────────────────────────────
        $history = $this->get_rate_history();
    ...
    private function get_referred_users($from_date = '', $to_date = '') {
        ...
        // ── Pre-compute whether fixed rewards are currently on ─
        $rewards_on = $this->is_rewards_enabled();
        // ─────────────────────────────────────────────────────
        ...
        foreach ($users as $user) {
            ...
            // ── Reward amount logic ───────────────────────────
            // • Already-redeemed rows: always show the real historical
            //   rate so the payout record stays intact.
            // • Pending / flagged / rejected rows: show 0 when the
            //   fixed reward system is disabled so no new payouts
            //   are implied. The normal BuyTap order bonus is
            //   completely separate and is not affected here.
            if ($status === 'redeemed') {
                $reward_amount = $this->get_reward_for_datetime($user->user_registered);
            } else {
                $reward_amount = $rewards_on
                    ? $this->get_reward_for_datetime($user->user_registered)
                    : 0;
            }
            // ─────────────────────────────────────────────────
    ...
    public function mark_referrals_rewarded() {
        ...
        // ── Block payout if rewards are disabled ──────────────
        if (!$this->is_rewards_enabled()) {
    ...
    public function render_admin_page() {
        ...
        <div class="wrap buytap-referral-rewards">
            ...
            <?php /* ══════════════════════════════════════════════════════════
                     NEW: Referral Rewards Status toggle
                     This controls ONLY the fixed plugin reward payouts.
                     The main BuyTap order referral bonus is not affected.
                  ══════════════════════════════════════════════════════════ */ ?>
            ...
            <?php /* ══ END: Referral Rewards Status toggle ══════════════ */ ?>
            ...
            <table class="widefat striped">
                ...
                <?php else: foreach ($summary as $item): ?>
                    <tr>
                        ...
                        <td>
                            <?php /* ── Show payout buttons ONLY when rewards are enabled ── */ ?>
                            <?php if ($rewards_enabled): ?>
                                ...
                                <?php endif; ?>
                            <?php else: ?>
                                <?php /* Rewards disabled — show neutral info, no buttons */ ?>
    ...
    public function export_csv() {
        ...
        // ── Add rewards status column to CSV ──────────────────
    ...
    public function enqueue_frontend_assets() {
        $css = "
            ...
            .bt-ref-paused-notice{background:rgba(251,191,36,.08);border:1px solid rgba(251,191,36,.3);border-radius:14px;padding:12px 16px;margin-bottom:12px;color:#fbbf24;font-size:13px;}
    ...
    public function shortcode_my_referrals_details($atts = []) {
        ...
        // ── When rewards are disabled, pending totals display as 0
        //    Redeemed history is unaffected (stored meta stays intact)
        $rewards_enabled = $this->is_rewards_enabled();
        // ─────────────────────────────────────────────────────
        ...
        $redeemed_total = array_sum(array_map(function($row) { return $row['reward_status'] === 'redeemed' ? floatval($row['reward_amount']) : 0; }, $my_rows));
        ...
        <div class="bt-ref-widget">
            <div class="bt-ref-mini">
                ...
                <div class="bt-ref-actions">
                    <?php if ($rewards_enabled): ?>
                        <p class="bt-ref-mini-reward">Pending reward: <strong style="color:#fbbf24;"><?php echo esc_html($currency . ' ' . number_format($pending_total, 2)); ?></strong></p>
                    <?php else: ?>
                        <?php /* When disabled show 0 for pending; show redeemed history unchanged */ ?>
                        <p class="bt-ref-mini-reward">Pending reward: <strong style="color:#6b7280;"><?php echo esc_html($currency . ' 0.00'); ?></strong></p>
                        <?php if ($redeemed_total > 0): ?>
                            <p class="bt-ref-mini-reward" style="font-size:12px;">Redeemed: <strong style="color:#22c55e;"><?php echo esc_html($currency . ' ' . number_format($redeemed_total, 2)); ?></strong></p>
                        <?php endif; ?>
                    <?php endif; ?>
                    <?php if ($mode === 'inline'): ?>
                        <div class="bt-ref-list">
                            <?php if (!$rewards_enabled && $pending_count > 0): ?>
                                <div class="bt-ref-paused-notice">⏸ Referral rewards are temporarily paused. Your referral count is being recorded. Redeemed rewards above are unaffected.</div>
                            <?php endif; ?>
                            <?php echo $this->render_frontend_rows($display_rows, $currency, $show_email); ?>
                        </div>
                    <?php else: ?>
            ...
            <?php if ($mode !== 'inline'): ?>
                <div id="<?php echo esc_attr($modal_id); ?>" class="bt-ref-overlay" aria-hidden="true">
                    <div class="bt-ref-modal" role="dialog" aria-modal="true">
                        ...
                        <?php if (!$rewards_enabled && $pending_count > 0): ?>
                            <div class="bt-ref-paused-notice">⏸ Referral rewards are temporarily paused. Your referral count is being recorded and your redeemed history is safe. Check back soon.</div>
                        <?php endif; ?>
                        <div class="bt-ref-stats">
                            ...
                            <div class="bt-ref-stat">
                                <div class="bt-ref-label">Pending</div>
                                <div class="bt-ref-value pending">
                                    <?php echo $rewards_enabled
                                        ? esc_html($currency . ' ' . number_format($pending_total, 2))
                                        : esc_html($currency . ' 0.00'); ?>
                                </div>
                            </div>
                            ...
                            <div class="bt-ref-stat">
                                ...
                                <div class="bt-ref-value">
                                    <?php echo $rewards_enabled
                                        ? esc_html($currency . ' ' . number_format($total, 2))
                                        : esc_html($currency . ' ' . number_format($redeemed_total, 2)); ?>
                                </div>
                            </div>
                        </div>
                        <div class="bt-ref-list">
