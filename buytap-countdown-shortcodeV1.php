...
$a = shortcode_atts([
  ...
  'open_dates'        => $opts['open_dates'],
	'key_suffix'        => '',
//   'heading'           => 'Next Shares Sale Starts In:',
// 	   'heading'          => '⏳ Launch Day: 27th October : ',
  'heading'  		  => '⏳BidWave Official Launch — 18th May:',
  ...
  <div id="<?= esc_attr($wrap_id); ?>">
    ...
    <style>
      ...
      #<?= $wrap_id; ?> .time-number {
        ...
        font-weight: bold;
        color: #00feaf; /* neon green countdown digits */
      }
      #<?= $wrap_id; ?> .time-label {
        font-size: 15px;
        margin-top: 40px;
...
(function () {
  ...
  if (mode === 'schedule') {
    // If exact open dates are set, obey ONLY those dates.
    if (openDates.length) {
      const explicit = nextExplicitOpen();
      return explicit;
    }

    // Only use daily open times when no exact dates are set.
