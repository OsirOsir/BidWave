...
class BuyTap_Referral_Reward_Reports_V5 {
    ...
    const OPTION_REWARDS_STATUS        = 'buytap_referral_rewards_status'; // 'enabled' | 'disabled'
    const NONCE_ACTION_SAVE_STATUS     = 'buytap_save_rewards_status';
    ...
    public function __construct() {
        ...
        add_action('admin_post_buytap_save_rewards_status', [$this, 'save_rewards_status']);
    ...
    public static function activate() {
        ...
        if (!get_option(self::OPTION_REWARDS_STATUS)) {
            update_option(self::OPTION_REWARDS_STATUS, 'enabled');
        }
    }
    ...
    // ══════════════════════════════════════════════════════════
    //  Rewards Status helpers
    //  These ONLY control the fixed plugin rewards.
    //  The main BuyTap order referral bonus system is untouched.
    // ══════════════════════════════════════════════════════════

    private function is_rewards_enabled() {
        return get_option(self::OPTION_REWARDS_STATUS, 'enabled') === 'enabled';
    }

    public function save_rewards_status() {
        if (!current_user_can('manage_options')) {
            wp_die('You do not have permission to change reward settings.');
        }
        check_admin_referer(self::NONCE_ACTION_SAVE_STATUS);

        $new_status = (isset($_POST['rewards_status']) && sanitize_key($_POST['rewards_status']) === 'disabled')
            ? 'disabled'
            : 'enabled';

        update_option(self::OPTION_REWARDS_STATUS, $new_status);

        wp_safe_redirect(admin_url('admin.php?page=buytap-referral-rewards&status_saved=1'));
        exit;
    }
    ...
    private function email_similarity_base($email) {
        ...
        $local = preg_replace('/[0-9._-]+/', '', $local);
        return $local . '@' . $domain;
    }
    ...
    private function get_referred_users($from_date = '', $to_date = '') {
        ...
        foreach ($users as $user) {
            ...
            // Always use the historical rate for the reward_amount field.
            // The frontend display layer decides whether to show it based on is_rewards_enabled().
    ...
    public function mark_referrals_rewarded() {
        ...
        if (!$this->is_rewards_enabled()) {
            wp_safe_redirect(admin_url('admin.php?page=buytap-referral-rewards&rewards_disabled_block=1'));
            exit;
        }
    ...
    public function render_admin_page() {
        ...
        $redeemed_payout = array_sum(array_map(function($row) { return $row['reward_status'] === 'redeemed' ? floatval($row['reward_amount']) : 0; }, $rows));
        $history = $this->get_rate_history();
        $rewards_enabled = $this->is_rewards_enabled();
        ?>
        <div class="wrap buytap-referral-rewards">
            ...
            <?php if (isset($_GET['status_saved'])): ?>
                <div class="notice notice-success is-dismissible"><p><strong>Referral Rewards Status updated.</strong> The change takes effect immediately.</p></div>
            <?php endif; ?>
            <?php if (isset($_GET['rewards_disabled_block'])): ?>
                <div class="notice notice-error is-dismissible"><p><strong>Action blocked.</strong> Referral Rewards are currently <strong>Disabled</strong>. Enable rewards first before marking payouts.</p></div>
            <?php endif; ?>
            ...
            <div style="background:<?php echo $rewards_enabled ? '#f0fdf4' : '#fef2f2'; ?>;border:2px solid <?php echo $rewards_enabled ? '#16a34a' : '#dc2626'; ?>;border-radius:6px;padding:18px 22px;margin:20px 0;max-width:980px;display:flex;align-items:center;justify-content:space-between;gap:20px;flex-wrap:wrap;">
                <div>
                    <h2 style="margin:0 0 4px;font-size:18px;color:<?php echo $rewards_enabled ? '#15803d' : '#b91c1c'; ?>;">
                        <?php echo $rewards_enabled ? '🟢 Referral Rewards: Enabled' : '🔴 Referral Rewards: Disabled'; ?>
                    </h2>
                    <p style="margin:0;font-size:13px;color:#555;">
                        <?php if ($rewards_enabled): ?>
                            Fixed referral reward payouts are <strong>active</strong>. Pending amounts are shown and payout buttons work normally.
                        <?php else: ?>
                            Fixed referral reward payouts are <strong>stopped</strong>. Pending amounts show as <?php echo esc_html($currency); ?> 0 and payout buttons are hidden. <strong>Existing payout history is untouched.</strong> The normal BuyTap order referral bonus continues working.
                        <?php endif; ?>
                    </p>
                </div>
                <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" onsubmit="return confirm('<?php echo $rewards_enabled ? 'Disable the fixed referral reward payouts? Referral tracking and the main BuyTap order bonus will continue normally.' : 'Re-enable the fixed referral reward payouts?'; ?>');">
                    <input type="hidden" name="action" value="buytap_save_rewards_status" />
                    <input type="hidden" name="rewards_status" value="<?php echo $rewards_enabled ? 'disabled' : 'enabled'; ?>" />
                    <?php wp_nonce_field(self::NONCE_ACTION_SAVE_STATUS); ?>
                    <button type="submit" class="button <?php echo $rewards_enabled ? 'button-secondary' : 'button-primary'; ?>" style="<?php echo $rewards_enabled ? 'border-color:#dc2626;color:#dc2626;' : 'background:#15803d;border-color:#15803d;'; ?>font-weight:700;">
                        <?php echo $rewards_enabled ? '⏸ Disable Referral Rewards' : '▶ Enable Referral Rewards'; ?>
                    </button>
                </form>
            </div>
            ...
            <div style="background:#fff;border:1px solid #dcdcde;padding:16px;margin:16px 0;max-width:980px;">
                ...
                <p><strong>Current reward:</strong> <?php echo esc_html($currency . ' ' . number_format($this->get_reward_for_datetime(current_time('mysql')), 2)); ?> per referred user
                <?php if (!$rewards_enabled): ?>
                    &nbsp;<span style="background:#fef2f2;color:#b91c1c;border:1px solid #fecaca;border-radius:4px;padding:2px 8px;font-size:12px;font-weight:700;">PAYOUTS DISABLED — amounts show as 0</span>
                <?php endif; ?>
                </p>
                <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                    ...
                    <table class="form-table" role="presentation">
                        <tr><th scope="row"><label for="reward_amount">New reward per referred user</label></th><td><input type="number" step="0.01" min="0" id="reward_amount" name="reward_amount" value="<?php echo esc_attr($this->get_reward_for_datetime(current_time('mysql'))); ?>" /> <span><?php echo esc_html($currency); ?></span></td></tr>
            ...
            <div style="display:flex;gap:16px;flex-wrap:wrap;margin:16px 0;">
                ...
                <div style="background:<?php echo $rewards_enabled ? '#fff' : '#fef2f2'; ?>;border:1px solid #dcdcde;padding:16px;min-width:220px;">
                    <strong>Pending Amount</strong><?php if (!$rewards_enabled): ?> <span style="font-size:11px;background:#fef2f2;color:#b91c1c;border:1px solid #fecaca;border-radius:3px;padding:1px 6px;font-weight:700;">DISABLED</span><?php endif; ?><br>
                    <span style="font-size:28px;font-weight:700;color:<?php echo $rewards_enabled ? '#b45309' : '#9ca3af'; ?>;"><?php echo esc_html($currency . ' ' . number_format($pending_payout, 2)); ?></span>
                </div>
            ...
            <?php if (!$rewards_enabled): ?>
                <div style="background:#fef9c3;border:1px solid #fde047;border-radius:6px;padding:12px 16px;margin-bottom:12px;max-width:980px;">
                    ⚠️ <strong>Referral Rewards are Disabled.</strong> Payout buttons are hidden. Referral tracking and the main BuyTap order bonus continue working normally.
                </div>
            <?php endif; ?>
            <table class="widefat striped">
                ...
                <?php if (empty($summary)): ?>
                    <tr><td colspan="11">No referrals found for this date range.</td></tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if ($rewards_enabled): ?>
                                ...
                                <?php endif; ?>
                            <?php else: ?>
                                <span style="color:#9ca3af;font-size:12px;">Payouts paused</span>
                                <?php if ($item['redeemed_count'] > 0): ?>
                                    <span style="color:#15803d;font-size:12px;margin-left:6px;">(<?php echo esc_html($item['redeemed_count']); ?> already redeemed)</span>
                                <?php endif; ?>
                            <?php endif; ?>
                        </td>
                    </tr>
            ...
            <table class="widefat striped">
                ...
                <tbody>
                <?php if (empty($rows)): ?>
                    <tr><td colspan="13">No referral details found.</td></tr>
                <?php else: foreach ($rows as $row): ?>
                    <tr>
                        ...
                        <td>
                            <strong><?php echo esc_html($currency . ' ' . number_format($row['reward_amount'], 2)); ?></strong>
                            <?php if (!$rewards_enabled && $row['reward_status'] !== 'redeemed'): ?>
                                <span style="font-size:10px;color:#9ca3af;display:block;">payouts paused</span>
                            <?php endif; ?>
                        </td>
    ...
    public function export_csv() {
        ...
        $rows = $this->get_referred_users($from, $to);
        $currency = $this->get_currency();
        $rewards_status_label = $this->is_rewards_enabled() ? 'Enabled' : 'Disabled';
        ...
        fputcsv($out, ['Date', 'Time', 'Referrer Name', 'Referrer Email', 'Referrer M-Pesa', 'Referral Code', 'Referred Name', 'Referred Email', 'Referred M-Pesa', 'Code Used', 'Reward Currency', 'Reward Amount', 'Reward Status', 'Rewards System Status', 'Fraud Flags', 'Fraud Reason Hints', 'Admin Recommendation', 'Review Status', 'Review Note', 'Redeemed At', 'Batch']);

        foreach ($rows as $row) {
            fputcsv($out, [$row['date'], $row['time'], $row['referrer_name'], $row['referrer_email'], $row['referrer_mobile'], $row['referrer_code'], $row['referred_name'], $row['referred_email'], $row['referred_mobile'], $row['promo_code_used'], $currency, number_format(floatval($row['reward_amount']), 2, '.', ''), $row['reward_status'], $rewards_status_label, implode(' | ', $row['fraud_flags']), implode(' | ', $row['fraud_reason_hints']), $row['fraud_recommendation'], $row['review_status'], $row['review_note'], $row['rewarded_at'], $row['rewarded_batch']]);
        }
    ...
    public function enqueue_frontend_assets() {
        $css = "
            ...
            .bt-ref-row{display:grid;gap:10px;align-items:center;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:14px;padding:12px;}
            .bt-ref-row.with-rewards{grid-template-columns:100px minmax(180px,1fr) 120px 120px;}
            .bt-ref-row.no-rewards{grid-template-columns:100px minmax(180px,1fr);}
            ...
            @media(max-width:760px){.bt-ref-stats{grid-template-columns:1fr 1fr}.bt-ref-row.with-rewards{grid-template-columns:1fr}.bt-ref-row.no-rewards{grid-template-columns:1fr}.bt-ref-title{font-size:24px}}
    ...
    // ══════════════════════════════════════════════════════════
    //  Frontend shortcode: [buytap_my_referrals_details]
    //
    //  When rewards ENABLED  → show full dashboard:
    //    total, pending count, redeemed count, pending reward amount,
    //    redeemed amount, all-time reward, KES per row, status badges.
    //
    //  When rewards DISABLED → show referral-only view:
    //    title, referral code, total count, date, name, email only.
    //    Hide: pending reward, redeemed amount, all-time reward,
    //           KES amounts, status badges, paused notice.
    // ══════════════════════════════════════════════════════════
    public function shortcode_my_referrals_details($atts = []) {
        ...
        // Counts — always shown regardless of rewards status.
        $total_count = count($my_rows);
        $pending_count = count(array_filter($my_rows, function($row) {
            return $row['reward_status'] === 'pending';
        }));
        $redeemed_count = count(array_filter($my_rows, function($row) {
            return $row['reward_status'] === 'redeemed';
        }));

        // Amounts — only computed when rewards are enabled (they are hidden when disabled).
        $pending_total  = $rewards_enabled
            ? array_sum(array_map(function($row) { return $row['reward_status'] === 'pending'  ? floatval($row['reward_amount']) : 0; }, $my_rows))
            : 0;
        $redeemed_total = $rewards_enabled
            ? array_sum(array_map(function($row) { return $row['reward_status'] === 'redeemed' ? floatval($row['reward_amount']) : 0; }, $my_rows))
            : 0;
        $total_reward   = $rewards_enabled
            ? array_sum(array_map(function($row) { return floatval($row['reward_amount']); }, $my_rows))
            : 0;
        ...
        <div class="bt-ref-widget">
            <div class="bt-ref-mini">
                ...
                <div class="bt-ref-mini-grid">
                    <?php /* Total count: always visible */ ?>
                    <div class="bt-ref-mini-stat">
                        <div class="bt-ref-mini-label">Total</div>
                        <div class="bt-ref-mini-value"><?php echo esc_html($total_count); ?></div>
                    </div>

                    <?php if ($rewards_enabled): ?>
                        <?php /* Pending count: shown when rewards enabled */ ?>
                        <div class="bt-ref-mini-stat">
                            <div class="bt-ref-mini-label">Pending</div>
                            <div class="bt-ref-mini-value" style="color:#fbbf24;"><?php echo esc_html($pending_count); ?></div>
                        </div>
                        <?php /* Redeemed count: shown when rewards enabled */ ?>
                        <div class="bt-ref-mini-stat">
                            <div class="bt-ref-mini-label">Redeemed</div>
                            <div class="bt-ref-mini-value" style="color:#22c55e;"><?php echo esc_html($redeemed_count); ?></div>
                        </div>
                    <?php endif; ?>
                </div>

                <div class="bt-ref-actions">
                    <?php if ($rewards_enabled): ?>
                        <?php /* Pending reward amount: shown only when rewards enabled */ ?>
                        <p class="bt-ref-mini-reward">
                            Pending reward: <strong style="color:#fbbf24;"><?php echo esc_html($currency . ' ' . number_format($pending_total, 2)); ?></strong>
                        </p>
                    <?php endif; ?>

                    <?php if ($mode === 'inline'): ?>
                        <div class="bt-ref-list">
                            <?php echo $this->render_frontend_rows($display_rows, $currency, $show_email, $rewards_enabled); ?>
                        </div>
                    <?php else: ?>
                        <button type="button" class="bt-ref-open" data-bt-ref-open="<?php echo esc_attr($modal_id); ?>">
            ...
            <?php if ($mode !== 'inline'): ?>
                <div id="<?php echo esc_attr($modal_id); ?>" class="bt-ref-overlay" aria-hidden="true">
                    <div class="bt-ref-modal" role="dialog" aria-modal="true">
                        ...
                        <?php if ($rewards_enabled): ?>
                            <?php /* Full reward stats: shown only when rewards enabled */ ?>
                            <div class="bt-ref-stats">
                                <div class="bt-ref-stat">
                                    <div class="bt-ref-label">Total referred</div>
                                    <div class="bt-ref-value"><?php echo esc_html($total_count); ?></div>
                                </div>
                                <div class="bt-ref-stat">
                                    <div class="bt-ref-label">Pending reward</div>
                                    <div class="bt-ref-value pending"><?php echo esc_html($currency . ' ' . number_format($pending_total, 2)); ?></div>
                                </div>
                                <div class="bt-ref-stat">
                                    <div class="bt-ref-label">Redeemed</div>
                                    <div class="bt-ref-value redeemed"><?php echo esc_html($currency . ' ' . number_format($redeemed_total, 2)); ?></div>
                                </div>
                                <div class="bt-ref-stat">
                                    <div class="bt-ref-label">All-time reward</div>
                                    <div class="bt-ref-value"><?php echo esc_html($currency . ' ' . number_format($total_reward, 2)); ?></div>
                                </div>
                            </div>
                        <?php else: ?>
                            <?php /* When disabled: show only total count, no reward figures */ ?>
                            <div class="bt-ref-stats" style="grid-template-columns:1fr;">
                                <div class="bt-ref-stat">
                                    <div class="bt-ref-label">Total referred</div>
                                    <div class="bt-ref-value"><?php echo esc_html($total_count); ?></div>
                                </div>
    ...
    //
    //  $rewards_enabled = true  → full row: date | name/email | KES amount | status badge
    //  $rewards_enabled = false → slim row: date | name/email only
    //                             (no KES amount, no status badge)
    // ══════════════════════════════════════════════════════════
    private function render_frontend_rows($rows, $currency, $show_email, $rewards_enabled = true) {
        ob_start();
        if (empty($rows)) {
            ...
            foreach ($rows as $row) {
                if ($rewards_enabled) {
                    // ── Full row with reward amount and status badge ──
                    $status = in_array($row['reward_status'], ['redeemed', 'flagged', 'rejected'], true)
                        ? $row['reward_status']
                        : 'pending';
                    $label = $status === 'redeemed'
                        ? 'Redeemed'
                        : ($status === 'flagged'
                            ? 'Under Review'
                            : ($status === 'rejected' ? 'Rejected' : 'Pending'));
                    ?>
                    <div class="bt-ref-row with-rewards">
                        <div class="bt-ref-date"><?php echo esc_html($row['date']); ?></div>
                        <div>
                            <div class="bt-ref-name"><?php echo esc_html($row['referred_name']); ?></div>
                            <?php if ($show_email): ?>
                                <div class="bt-ref-email"><?php echo esc_html($row['referred_email']); ?></div>
                            <?php endif; ?>
                        </div>
                    ...
                    // ── Slim row — no reward amounts, no status badges ──
                    ?>
                    <div class="bt-ref-row no-rewards">
                        <div class="bt-ref-date"><?php echo esc_html($row['date']); ?></div>
                        <div>
                            <div class="bt-ref-name"><?php echo esc_html($row['referred_name']); ?></div>
                            <?php if ($show_email): ?>
