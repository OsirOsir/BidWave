 ...
 * Description: Adds referral reward history, payout marking, daily referral tracking, referred-user details, payout summaries, CSV export, and frontend popup referral cards, fraud flags, duplicate M-Pesa blocking, and self-referral protection, and admin approve/reject review workflow for the existing BuyTap referral system.
 * Version: 5.3.0
...
class BuyTap_Referral_Reward_Reports_V5 {
    ...
    public function render_admin_page() {
        ...
        <div class="wrap buytap-referral-rewards">
            ...
            <table class="widefat striped">
                <thead><tr><th>Date</th><th>Time</th><th>Referrer</th><th>Referrer M-Pesa</th><th>Referred Person</th><th>Referred Email</th><th>Reward</th><th>Status</th><th>Fraud Flags</th><th>Review</th><th>Redeemed At</th><th>Code Used</th></tr></thead>
                <tbody>
                <?php if (empty($rows)): ?>
    ...
    public function export_csv() {
        ...
        fputcsv($out, ['Date', 'Time', 'Referrer Name', 'Referrer Email', 'Referrer M-Pesa', 'Referral Code', 'Referred Name', 'Referred Email', 'Referred M-Pesa', 'Code Used', 'Reward Currency', 'Reward Amount', 'Reward Status', 'Fraud Flags', 'Review Status', 'Review Note', 'Redeemed At', 'Batch']);

        foreach ($rows as $row) {
            fputcsv($out, [$row['date'], $row['time'], $row['referrer_name'], $row['referrer_email'], $row['referrer_mobile'], $row['referrer_code'], $row['referred_name'], $row['referred_email'], $row['referred_mobile'], $row['promo_code_used'], $currency, number_format(floatval($row['reward_amount']), 2, '.', ''), $row['reward_status'], implode(' | ', $row['fraud_flags']), $row['review_status'], $row['review_note'], $row['rewarded_at'], $row['rewarded_batch']]);
        }
